...
Default path behavior when `--path` is omitted:

- If `./.brewva` exists, create under `./.brewva/skills/packs/<skill-name>`
- Otherwise create under global `~/.config/brewva/skills/packs/<skill-name>` (or env-resolved global root)

The script:

- Resolves install path from `--path` or defaults above
