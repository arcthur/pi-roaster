...
describe("S-009a cognition note tool flow", () => {
  test("cognition_note records and lists operator teaching artifacts", async () => {
    ...
    expect(extractTextContent(recorded).includes("Recorded procedure cognition note")).toBe(true);
    ...
    expect(listedText.includes("# Cognition Notes")).toBe(true);
    expect(listedText.includes("verification-standard-implementation")).toBe(true);
  ...
  test("cognition_note episode can carry an explicit session scope for future rehydration", async () => {
