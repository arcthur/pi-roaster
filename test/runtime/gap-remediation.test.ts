...
describe("Gap remediation: event stream and context budget", () => {
  ...
  test("keeps pending resume hint when injection is dropped by hard limit", () => {
    const workspace = createWorkspace("context-hard-limit-resume-hint");
    writeConfig(workspace, createConfig({}));
    ...
    const dropped = runtimeB.buildContextInjection(resumedSession, "resume task", {
      tokens: 195_000,
      contextWindow: 200_000,
      percent: 0.975,
    });
    expect(dropped.accepted).toBe(false);
    runtimeB.onTurnStart(resumedSession, 1);

    const recovered = runtimeB.buildContextInjection(resumedSession, "resume task", {
      tokens: 600,
      contextWindow: 200_000,
      percent: 0.3,
    });
    expect(recovered.accepted).toBe(true);
    expect(recovered.text.includes("[ResumeHint]")).toBe(true);
    expect(recovered.text.includes("Session recovered from interruption")).toBe(true);
  });

  test("prefers resumeHintInjectionEnabled over legacy resumeHintInSystemPrompt", () => {
    const workspace = createWorkspace("context-resume-hint-flag");
    const config = createConfig({});
    config.infrastructure = {
      ...config.infrastructure,
      interruptRecovery: {
        ...
        resumeHintInjectionEnabled: false,
      },
    };
    writeConfig(workspace, config);
    ...
    const injection = runtimeB.buildContextInjection(resumedSession, "resume task", {
      tokens: 400,
      contextWindow: 4000,
      percent: 0.1,
    });
    expect(injection.accepted).toBe(true);
    expect(injection.text.includes("[ResumeHint]")).toBe(false);
  });

  test("supports legacy resumeHintInSystemPrompt when new key is absent", () => {
    const workspace = createWorkspace("context-resume-hint-legacy");
    ...
    const legacyOnly = {
      ...
      infrastructure: {
        ...base.infrastructure,
        interruptRecovery: {
          enabled: base.infrastructure.interruptRecovery.enabled,
          snapshotsDir: base.infrastructure.interruptRecovery.snapshotsDir,
          gracefulTimeoutMs: base.infrastructure.interruptRecovery.gracefulTimeoutMs,
          resumeHintInSystemPrompt: false,
          sessionHandoff: base.infrastructure.interruptRecovery.sessionHandoff,
        },
      },
    };
...
describe("Gap remediation: interrupt recovery snapshot", () => {
  test("restores active skill and verification state from snapshot", () => {
    const workspace = createWorkspace("snapshot");
    writeConfig(workspace, createConfig({}));
    const sessionId = "snapshot-1";
...
patching`,
    ...
    const state = recovered.verification.stateStore.get(sessionId);
    expect(state.evidence.length).toBeGreaterThan(0);
  });

  test("clears stale active parallel runs during restore", () => {
    const workspace = createWorkspace("snapshot-parallel");
    writeConfig(
      workspace,
      createConfig({
        parallel: {
    ...
    const resumed = recovered.queryEvents(sessionId, { type: "session_resumed", last: 1 })[0];
    const payload = resumed?.payload as { droppedActiveRuns?: number } | undefined;
    expect(payload?.droppedActiveRuns).toBe(1);
  });

  test("startup restore imports latest interrupted snapshot when session id changed", () => {
    const workspace = createWorkspace("snapshot-startup-import");
    writeConfig(workspace, createConfig({}));
    ...
    const resumed = runtimeB.queryEvents("new-session", { type: "session_resumed", last: 1 })[0];
    const payload = resumed?.payload as
      | { imported?: boolean; snapshotSessionId?: string; importedPatchSets?: number }
      | undefined;
    expect(payload?.imported).toBe(true);
    expect(payload?.snapshotSessionId).toBe(interruptedSession);
    expect(payload?.importedPatchSets).toBe(0);

    expect(runtimeB.snapshots.load(interruptedSession)).toBeUndefined();
    const consumed = runtimeB.snapshots.load("new-session");
    expect(consumed).toBeDefined();
    expect(consumed?.interrupted).toBe(false);
  });

  test("startup import carries patch history so undo works in new session id", () => {
    const workspace = createWorkspace("snapshot-startup-import-undo");
    writeConfig(workspace, createConfig({}));
    ...
    runtimeA.trackToolCallStart({
      ...
      toolCallId: "import-1",
      toolName: "edit",
      args: { file_path: "src/import-undo.ts" },
    });
    ...
    const rollback = runtimeB.rollbackLastPatchSet("new-session");
...
describe("Gap remediation: cost view and budget linkage", () => {
  ...
  test("restores cost snapshot and keeps budget continuity across startup recovery", () => {
    const workspace = createWorkspace("cost-restore");
    const config = createConfig({});
    config.infrastructure = {
      ...config.infrastructure,
      costTracking: {
    ...
    const before = runtimeB.getCostSummary("cost-new");
    expect(before.totalCostUsd).toBeCloseTo(0.003, 8);
    expect(before.totalTokens).toBe(50);

    runtimeB.onTurnStart("cost-new", 3);
    runtimeB.recordAssistantUsage({
      sessionId: "cost-new",
      model: "test/model",
      inputTokens: 20,
      outputTokens: 10,
    ...
    const after = runtimeB.getCostSummary("cost-new");
    expect(after.totalCostUsd).toBeCloseTo(0.005, 8);
    expect(after.totalTokens).toBe(80);
  });
