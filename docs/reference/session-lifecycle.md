...
3. Register lifecycle handlers (profile-dependent)
   - extensions-enabled: full extension stack from `packages/brewva-gateway/src/runtime-plugins/index.ts`
...
- `--no-addons`: keeps runtime-core safety/evidence hooks plus minimal core context status injection, while extension presentation hooks remain disabled
